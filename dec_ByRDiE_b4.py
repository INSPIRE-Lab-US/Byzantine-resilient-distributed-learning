...
# Function to generate the graph of our decentralized computing network
...
class byrdie_initialization:
    ...
    def save(self, r):
        with open('./checkpt/ByRDiE_b4_checkpt_ini_%d.pickle'%r, 'wb') as handle:
            pickle.dump(self, handle)
        
def chekpt_read(r):
    with open('./checkpt/ByRDiE_b4_checkpt_ini_%d.pickle'%r, 'rb') as handle: 
        ini = pickle.load(handle)
    with open('./checkpt/ByRDiE_b4_checkpt_current_%d.pickle'%r, 'rb') as handle: 
...
if __name__ == "__main__":
    ...
    try:
        ...
        para = experiment_parameters(agents=20, dataset='MNIST', localsize_N=2000, iteration=100,
                                  screen=True, b=4, Byzantine='random', stepsize = 1e-1)
        loaded = False    
    #Generate the graph
   ...
   #Initialization
    ...
    for iteration in range(para.T):
       ...
       #node update using Adam    
        ...
        print(iteration)
        save.append(accuracy)
        with open('./result/ByRDiE_b4_%d.pickle'%r, 'wb') as handle:
