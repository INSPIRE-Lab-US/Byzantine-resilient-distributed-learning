...
for iteration in range(para.T):
    #Communication 
    para.communication(w_nodes, neighbors, sess, para.b, para.screen)
    ...
    para.node_update(w_nodes[para.b:], local_set, sess, stepsize=para.stepsize/(iteration+1))


sess.close()
end = time.time()
print(f'Monte Carlo {monte_trial} Done!\n Time elapsed {end-start} seconds\n')
with open(f'./result/BRIDGE/result_BRIDGE_faultless_{monte_trial}.pickle', 'wb') as handle:
